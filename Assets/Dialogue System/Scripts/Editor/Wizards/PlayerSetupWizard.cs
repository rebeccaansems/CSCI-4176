	...
	public class PlayerSetupWizard : EditorWindow {
		...
		private void DrawControlStage() {
			...
			ControlStyle controlStyle = (simpleController != null)
				? ControlStyle.SimpleThirdPersonShooter
			...
			switch (controlStyle) {
			case ControlStyle.SimpleThirdPersonShooter:
		...
		private void DrawSimpleControllerSection(PixelCrushers.DialogueSystem.SimpleController simpleController) {
			...
			if ((simpleController.idle == null) || (simpleController.runForward == null)) {
				EditorGUILayout.HelpBox("The player uses very simple third-person shooter style controls using the legacy animation system. At a minimum, Idle and Run animations are required. Click Select Player to customize further.", MessageType.Info);
			}
		...
		private void DrawDisableControlsSection() {
			...
			if (setEnabledFlag) {
				if (enabler == null) enabler = pcObject.AddComponent<SetComponentEnabledOnDialogueEvent>();
