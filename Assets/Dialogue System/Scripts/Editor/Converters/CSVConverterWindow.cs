...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class CSVConverterWindow : AbstractConverterWindow
    {
        ...
        {
            Debug.Log("Dialogue System CSV Converter: Copying source to dialogue database.");
            numWarnings = 0;
            var hadError = false;
            ...
            try
            {
                try
                {
                    ...
                    {
                        Debug.Log("Dialogue System CSV Converter: Conversations section was after DialogueEntries section. Going back to read DialogueEntries now...");
                        LoadSourceFile();
                        safeguard = 0;
                        while (!IsSourceAtEnd() && (safeguard < MaxIterations) && !cancel)
                        {
                ...
                {
                    Debug.LogError(string.Format("Dialogue System CSV Converter: CSV conversion failed: {1}\nLine {2}: {3}", DialogueDebug.Prefix, e.Message, currentLineNumber, currentSourceLine));
                    hadError = true;
                }
            }
            finally
            {
                ...
                if (hadError)
                {
                    var message = "Dialogue System CSV Converter: There was a fatal error that prevented conversion of " + prefs.sourceFilename + ".";
                    Debug.LogError(message);
                    EditorUtility.DisplayDialog("CSV Conversion Error", message, "OK");
                }
                else if (numWarnings > 0)
                {
                    var message = "Dialogue System CSV Converter: There were " + numWarnings + " warnings during conversion of " + prefs.sourceFilename + ".";
                    Debug.LogWarning(message);
                    EditorUtility.DisplayDialog("CSV Conversion Warnings", message, "OK");
                }
            }
        }
        ...
        {
            Debug.Log("Dialogue System CSV Converter: Reading database properties");
        ...
        private void ReadAssets<T>(List<T> assets, bool add) where T : Asset, new()
        {
            ...
            Debug.Log(string.Format("Dialogue System CSV Converter: {0} {1} section", (add ? "Reading" : "Skipping"), typeName));
        ...
        {
            Debug.Log("Dialogue System CSV Converter: " + (add ? "Reading" : "Skipping") + " OutgoingLinks section");
            ...
            while (!(IsSourceAtEnd() || AssetTypeHeadings.Contains(GetFirstField(PeekNextSourceLine()))))
            {
                ...
                if (add)
                {
                    ...
                    if (entry == null)
                    {
                        Debug.LogWarning(string.Format("Dialogue System: CSV conversion error: dialogue entry {0}.{1} referenced in an outgoing link was not found.", link.originConversationID, link.originDialogueID));
                        numWarnings++;
                    }
                    else
                    {
